...
| Package distribution | No VectorWarp packages | Native DEB/RPM packaging, release automation and one signed APT/DNF repository implementation | Fresh Fedora 44 x86-64 RPM built, inspected and replay-tested after extraction. Real format/signature fixtures and Jammy/Noble/Resolute APT indexes pass. Ubuntu 26.04 has a pinned hosted-build definition and DragonOS has metadata-selector tests, but no DragonOS ISO boot, clean-host install, or hosted package build is yet verified. **No published repository or verified hosted release yet.** |
