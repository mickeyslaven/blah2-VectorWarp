...
| Package distribution | No VectorWarp packages | Ten native DEB/RPM targets across Ubuntu, Debian and Fedora; release automation and one signed APT/DNF repository implementation | Fedora 44 x86-64 (amd64 / x86_64) RPM built, inspected and replay-tested after extraction. Real format/signature fixtures and Jammy/Noble/Resolute/Trixie APT indexes pass. Debian/Pi/DragonOS selector fixtures pass, including 32-bit-userland rejection. Hosted package installation and Pi/DragonOS hardware validation remain pending. **No published repository or verified hosted release yet.** |

Hosted Ubuntu 24.04 x86-64 (amd64 / x86_64) and ARM64 (arm64 / aarch64)
compilation, C++ unit tests and full-processor
replay passed on integration commit `cb54581`. The workflow then failed in its
Python test launcher; this is partial evidence, not a passing full CI run or
package-installation result. See the [hosted run](https://github.com/mickeyslaven/blah2-VectorWarp/actions/runs/34527456219).
