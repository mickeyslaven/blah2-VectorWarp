...
for host in ('strix', 'nvidia', 'pavilion'):
    ...
    assert len(runs) == {'strix': 42, 'nvidia': 28, 'pavilion': 40}[host], (host, len(runs))
    assert contract['fork_source_commit'] == '8ba6e1330fad9fcdbe8a5a54134a712d0959775e'
    assert contract['source_freeze_sha256'] == 'ccbd2ec1c379310b0df1be1857570d93dd5306a6b435c63c8e785b7c8084dab6'
    for case in ('pair-200ms-800hz', 'pair-200ms-2400hz'):
        for repeat in (1, 2):
            old = folder / f'{case}-vectorwarp-before-cpu-auto-r{repeat}.outputs.jsonl'
            new = folder / f'{case}-vectorwarp-cpu-auto-r{repeat}.outputs.jsonl'
            assert old.read_bytes() == new.read_bytes(), (host, case, repeat, 'CPU output changed')
    groups = {}
    for run in runs:
