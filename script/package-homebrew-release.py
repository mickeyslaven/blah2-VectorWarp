...
```sh
brew tap {TAP}
brew install --build-from-source {TAP}/vectorwarp
vectorwarp
```
...
```sh
brew update
brew upgrade --build-from-source {TAP}/vectorwarp-heimdall {TAP}/vectorwarp
```

Restart any active VectorWarp instance after upgrading either formula.
...
```sh
brew services start {TAP}/vectorwarp
```

## Remove

```sh
brew services stop {TAP}/vectorwarp
vectorwarp stop
brew uninstall {TAP}/vectorwarp
brew uninstall {TAP}/vectorwarp-heimdall
brew untap {TAP}
```
