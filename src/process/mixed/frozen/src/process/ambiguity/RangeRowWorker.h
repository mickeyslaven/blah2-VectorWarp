...
public:
  ...
  struct Job {
    ...
    const Complex* rawX = nullptr;
    const Complex* rawY = nullptr;
    uint32_t rawXSamples = 0;
    uint32_t rawYSamples = 0;
...
private:
  ...
  void execute(const Job& job) {
    if ((!job.x && !job.rawX) || (!job.y && !job.rawY) || !job.rows || job.fft != fftLength_ ||
        !job.corr || !job.delays ||
        job.first >= job.last || job.corr > job.fft || !job.fs ||
        job.delayMin <= -int64_t(job.fft) ||
        int64_t(job.delayMin)+job.delays-1 >= job.fft ||
        (job.x ? job.last * uint64_t(job.corr) > job.x->size() : job.last * uint64_t(job.corr) > job.rawXSamples) ||
        (!job.filteredY && (job.y ? job.last * uint64_t(job.corr) > job.y->size() : job.last * uint64_t(job.corr) > job.rawYSamples)) ||
    ...
    for (uint32_t row = job.first; row < job.last; ++row) {
      ...
      for (uint32_t j = 0; j < job.corr; ++j) {
        input_[j] = job.rotatedX ? job.rotatedX[rotatedIndex] : (job.rawX ? job.rawX[base+j] : (*job.x)[base+j]);
        ...
        input_[job.fft+j] = job.filteredY ? job.filteredY[base+j] : (job.rawY ? job.rawY[base+j] : (*job.y)[base+j]);
      }
