...
struct WienerHopf::Impl {
  ...
  bool process_initialized(bool borrowOutput, IqData* outputOwner,
      std::chrono::steady_clock::time_point inputStarted,
      std::chrono::steady_clock::time_point inputFinished) {
    viewReady = false;
#ifdef VECTORWARP_MIXED_FIR_BENCH
#ifdef VECTORWARP_GPU_BLOCKED_FULL_BENCH
    ...
    if (borrowOutput) { ++generation; viewReady = true; }
    else {
      if (!outputOwner) throw std::logic_error("Bounded clutter input requires borrowed output");
      outputOwner->assign_complex(y.get(), samples);
    }
    if (contiguousLog) {
  ...
  bool process(IqData* reference, IqData* surveillance, bool borrowOutput) {
    viewReady = false;
    if (!reference || !surveillance || reference->get_length() != samples ||
    ...
    const bool result = process_initialized(borrowOutput, surveillance, inputStarted,
      std::chrono::steady_clock::now());
    return result;
  }
  bool process_input(const Complex* reference, uint32_t referenceSamples,
      const Complex* surveillance, uint32_t surveillanceSamples, bool borrowOutput) {
    viewReady = false;
    if (!reference || !surveillance || referenceSamples != samples || surveillanceSamples != samples)
      throw std::invalid_argument("Clutter input length differs from CPI");
    if (!borrowOutput)
      throw std::invalid_argument("Bounded clutter input requires borrowed output");
    const auto inputStarted = std::chrono::steady_clock::now();
    vectorwarp_clutter::copy_rotation(reference, referenceSamples, samples, delayMin, x.get());
    std::copy_n(surveillance, samples, y.get());
    return process_initialized(borrowOutput, nullptr, inputStarted, std::chrono::steady_clock::now());
  }
  bool process_paired_i16(const int16_t* input, uint32_t sampleCount,
      uint32_t referenceChannel) {
    if (!input || sampleCount != samples || referenceChannel > 1)
      throw std::invalid_argument("Paired clutter input differs from CPI");
    viewReady = false;
    const auto inputStarted = std::chrono::steady_clock::now();
    const uint32_t surveillanceChannel = referenceChannel ^ 1;
    const int64_t initial = (-int64_t(delayMin)) % int64_t(samples);
    uint32_t sourceIndex = static_cast<uint32_t>(initial < 0 ? initial + samples : initial);
    for (uint32_t written = 0; written < samples;) {
      const uint32_t count = std::min(samples - written, samples - sourceIndex);
      const int16_t* source = input + uint64_t(sourceIndex) * 4 + referenceChannel * 2;
      for (uint32_t i = 0; i < count; ++i, source += 4)
        x.get()[written + i] = {double(source[0]), double(source[1])};
      written += count;
      sourceIndex = 0;
    }
    const int16_t* source = input + surveillanceChannel * 2;
    for (uint32_t i = 0; i < samples; ++i, source += 4)
      y.get()[i] = {double(source[0]), double(source[1])};
    return process_initialized(true, nullptr, inputStarted, std::chrono::steady_clock::now());
  }
  ...
  uint32_t cpu_worker_slots() const {
    return static_cast<uint32_t>(correlationWorkers.size()) + 1;
  }
};
...
uint32_t WienerHopf::cpu_worker_slots() const { return impl_->cpu_worker_slots(); }
bool WienerHopf::process(IqData* x, IqData* y, bool borrowOutput) { return impl_->process(x,y,borrowOutput); }
bool WienerHopf::process_borrowed_input(const std::complex<double>* reference,
  uint32_t referenceSamples, const std::complex<double>* surveillance,
  uint32_t surveillanceSamples, bool borrowOutput) {
  return impl_->process_input(reference, referenceSamples, surveillance, surveillanceSamples, borrowOutput);
}
bool WienerHopf::process_borrowed_paired_i16(const int16_t* input,
  uint32_t sampleCount, uint32_t referenceChannel) {
  return impl_->process_paired_i16(input, sampleCount, referenceChannel);
}
