...
struct OwlGpuFilter::Impl {
 uint32_t samples,taps,fft,totalBlocks,blocks,outputs,percent,tile;
 ...
 bool active=false,referenceActive=false,cycleOpen=false,poisoned=false,earlyReference;
 std::ofstream log;uint64_t frame=0;
 vectorwarp_gpu_bench::ReadbackMode readbackMode;
 std::vector<std::complex<float>> readback;
 ...
 Impl(uint32_t n,uint32_t b,uint32_t k,uint32_t share,bool early):samples(n),taps(b),fft(k),percent(share),earlyReference(early),
  readbackMode(vectorwarp_gpu_bench::readbackMode(std::getenv("VECTORWARP_GPU_FIR_READBACK"))){
  if(!samples||!taps||taps>fft||samples>10000000||
     (fft!=1024&&fft!=2048&&fft!=4096)||
     (percent!=25&&percent!=50&&percent!=75&&percent!=100))throw std::runtime_error("Invalid bounded GPU FIR geometry");
  const uint32_t hop=fft-taps+1;
  totalBlocks=(samples+hop-1)/hop;
  blocks=std::max(1u,uint32_t(uint64_t(totalBlocks)*percent/100));
  outputs=std::min<uint64_t>(samples,uint64_t(blocks)*hop);
  ...
  if(readbackMode==vectorwarp_gpu_bench::ReadbackMode::cached)readback.resize(outputs);
  else if(readbackMode==vectorwarp_gpu_bench::ReadbackMode::chunked)
   readback.resize(std::min<uint32_t>(outputs,16384));
  ...
  if(const char* path=std::getenv("OWL_GPU_FILTER_LOG")){log.open(path,std::ios::app);if(!log)throw std::runtime_error("GPU timing log open failed");log<<std::setprecision(12);}
  std::cerr<<"OWL_GPU_FILTER device="<<it->info.name<<" fft="<<fft<<" lut="<<lut<<" warp="<<plan->app.configuration.warpSize<<" coalesced="<<plan->app.configuration.coalescedMemory<<" percent="<<percent<<" total_blocks="<<totalBlocks<<" blocks="<<blocks<<" tile="<<tile<<" output_samples="<<outputs<<" effective_gpu_percent="<<(100.0*outputs/samples)<<" properties="<<input->properties<<" early_reference="<<earlyReference<<" readback_mode="<<vectorwarp_gpu_bench::readbackName(readbackMode)<<" output_properties="<<output->properties<<"\n";
 }
 ~Impl(){
  if(context){
 ...
 void finish(std::complex<double>* y){
  ...
  const auto beforeInvalidate=Clock::now();output->invalidate(0,output->bytes);
  const double invalidateMs=elapsed(beforeInvalidate);
  const auto* p=static_cast<const std::complex<float>*>(output->mapped);
  const auto phases=vectorwarp_gpu_bench::readbackSubtract(
   p,outputs,readback,y,readbackMode);
  ...
  if(log)log<<"{\"frame\":"<<++frame<<",\"gpu_samples\":"<<outputs<<",\"total_samples\":"<<samples<<",\"effective_gpu_percent\":"<<(100.0*outputs/samples)
   <<",\"requested_gpu_percent\":"<<percent<<",\"fft_length\":"<<fft
   <<",\"gpu_blocks\":"<<blocks<<",\"total_blocks\":"<<totalBlocks
   <<",\"cpu_samples\":"<<(samples-outputs)
   ...
   <<",\"readback_mode\":\""<<vectorwarp_gpu_bench::readbackName(readbackMode)<<"\""
   <<",\"invalidate_ms\":"<<invalidateMs<<",\"mapped_copy_ms\":"<<phases.mappedCopyMs
   <<",\"finite_check_ms\":"<<phases.finiteCheckMs
   <<",\"fp64_subtract_ms\":"<<phases.fp64SubtractMs
   <<",\"filter_tail_ms\":"<<tail<<",\"reference_host_plus_tail_ms\":"<<referenceHostMs+tail
   <<",\"reference_lifetime_ms\":"<<elapsed(referenceStarted)<<"}\n"<<std::flush;
 }

};
OwlGpuFilter::OwlGpuFilter(uint32_t samples,uint32_t taps,uint32_t fftLength,uint32_t percent,bool earlyReference)
    try : impl_(std::make_unique<Impl>(samples,taps,fftLength,percent,earlyReference))
{ std::cerr << "OWL_GPU_FILTER status=active backend=vulkan share="<<percent<<" fft="<<fftLength<<"\n"; }
